            ...
            try:
                ...
                if is_10bit:
                    ...
                    if len(raw_bytes) != width * height:
                        print(f"[{label_name}] Error: Tamaño de frame incorrecto: {len(raw_bytes)}")
                        continue
                    bayer_2d = raw_bytes.reshape((height, width))
                    
                # --- PROCESAMIENTO DE COLOR Y LUZ ---
                # 1. Corrección del tinte verde (Matriz RGGB)
                color = cv2.cvtColor(bayer_2d, cv2.COLOR_BayerRG2BGR)
                
                # 2. AUTO-EXPOSICIÓN DIGITAL (Magia)
                # Estiramos los píxeles oscuros a blancos brillantes
                color = cv2.normalize(color, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
                
                # 3. Reducción rápida para no colapsar la red
                final_img = cv2.resize(color, (640, 360))
                
            except Exception as e:
                print(f"[{label_name}] Error matemático procesando frame: {e}")
                continue
